...
public class V1_Himno {	
    ...
    static {
    	Himno[] himnosarr=new Himno[]{
    		new Himno(1,407,"Castillo fuerte","Castillo fuerte es nuestro Dios,<Defensa y buen escudo.<Con su poder nos librará< En todo trance agudo.< Con furia y con afán< Acósanos satán:< Por armas deja ver< Astucia y gran poder;< Cual él no hay en la tierra.> Nuestro valor es nada aquí,< Con él todo es perdido;< Mas con nosotros luchará< De Dios el escogido.< Es nuestro Rey Jesús,< El que venció en la cruz,< Señor y Salvador,< Y siendo El solo Dios,< El triunfa en la batalla.> Y si demonios mil están< Prontos a devorarnos,< No temeremos, porque Dios< Sabrá cómo ampararnos.< ¡Que muestre su vigor< Satán, y su furor!< Dañarnos no podrá,< Pues condenado es ya< Por la Palabra Santa.> Esa palabra del Señor,< Que el mundo no apetece,< Por el Espíritu de Dios< Muy firme permanece.< Nos pueden despojar< De bienes, nombre, hogar,< El cuerpo destruir,< Mas siempre ha de existir< De Dios el Reino eterno. Amén."),
    		...
    		new Himno(3,927,"Sublime Gracia","Sublime Gracia del Señor< que a un infeliz salvo < Fui ciego mas hoy miro yo<  perdido y El me hallo > Su Gracia me enseño a tener < mis dudas ahuyento < Oh cuan precioso fue a mi ser < cuando el me transformo > En los peligros y aflicción < que yo he tenido aquí < su gracia siempre me libró < y me guiará feliz > Y cuando en Sión por siglos mil < brillando este cual sol < yo cantare por siempre ahí < su amor que me salvo"),
    ...
    @GET
    @Path("/updateHimno/{id}/{numero}/{name}/{lyrics}")
    public Collection<Himno> updateHimno(@PathParam("id") int id,@PathParam("numero") String numero,@PathParam("name") String name,@PathParam("lyrics") String lyrics) throws ParseException {    	    	
    	Himno new_himno = new Himno(id,Integer.parseInt(numero),name, lyrics);
    	Collection<Himno> result = new ArrayList<Himno>();    	
    	if (new_himno != null)
        {    		
    		himnos.put(String.valueOf(new_himno.getId()), new_himno);
